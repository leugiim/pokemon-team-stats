import { Team, Match, PokemonSet } from './types'
...
export function computeTeamStats(team: Team, matches: Match[]): TeamStats {
  ...
  const pokeStats: PokeStat[] = [...baseDisplayMap.entries()].map(([base, displayName]) => {
  ...
  return { decided, total, wins, wr: winrate(matches), allPokeNames, pokeStats, leadStats, enemyStats, enemyLeadStats }
}

// Filtros de las tablas de rivales
export function filterEnemyStats(stats: NamedStat[], nameFilter: string, minMatches: number): NamedStat[] {
  return stats
    .filter(s => s.total >= minMatches)
    .filter(s => !nameFilter || s.name.toLowerCase().includes(nameFilter.toLowerCase()))
}

export function filterEnemyLeadStats(stats: LeadStat[], nameFilter: string, minMatches: number): LeadStat[] {
