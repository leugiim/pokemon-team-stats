...
import {
  computeTeamStats, filterEnemyLeadStats, filterEnemyStats, filterMatches, megaBase, winrate,
} from '../src/stats'
import { jsonToHistory } from '../src/utils/matchIO'
...
const team: Team = {
  ...
  pokemon: parsePokepaste(read('team.paste.txt')),
  createdAt: 0,
}
...
function match(over: Partial<Match>): Match {
  return {
    id: 'x', teamId: team.id, date: 0, result: 'win', teamRoster: [], selection: [], lead: [],
    rivalTeam: [], rivalSelection: [], rivalLead: [], notes: '', ...over,
  }
}
...
describe('winrate', () => {
  ...
  it('ignores ongoing matches and rounds', () => {
    const ms = [match({ result: 'win' }), match({ result: 'win' }), match({ result: 'loss' }), match({ result: 'ongoing' })]
...
describe('computeTeamStats', () => {
  ...
  it('counts a duplicated rival pokemon once per match', () => {
    const s = computeTeamStats(team, [match({ result: 'loss', rivalTeam: ['Incineroar', 'Incineroar'] })])
  ...
  it('excludes ongoing matches from every stat', () => {
    const s = computeTeamStats(team, [match({ result: 'ongoing', selection: ['Charizard'], lead: ['A', 'B'], rivalTeam: ['X'], rivalLead: ['X', 'Y'] })])
    expect(s).toMatchObject({ total: 0, wins: 0, wr: null, leadStats: [], enemyStats: [], enemyLeadStats: [] })
  })
  ...
  it('merges pokemon with the same nickname (known limitation)', () => {
    const dup: Team = {
      ...team,
      pokemon: [
        { name: 'Rotom-Wash', nickname: 'Dup', item: '', ability: '', nature: '', evs: {}, moves: [] },
        { name: 'Rotom-Heat', nickname: 'Dup', item: '', ability: '', nature: '', evs: {}, moves: [] },
      ],
    }
...
describe('filters', () => {
  const ms: Match[] = [
    match({ id: '1', result: 'win', selection: ['Charizard', 'Sparky'], rivalTeam: ['Incineroar', 'Rillaboom'] }),
    match({ id: '2', result: 'loss', selection: ['Charizard-Mega-Y'], rivalTeam: ['Flutter Mane'] }),
  ...
  it('filters by own pokemon, matching Mega with base', () => {
    ...
    expect(ids(filterMatches(ms, { result: '', own: 'Charizard-Mega-Y', rival: '' }))).toEqual(['1', '2'])
  })
  ...
  it('filters enemy leads by any member name', () => {
    ...
    expect(filterEnemyLeadStats(leads, 'urshi', 1).map(s => s.lead)).toEqual(['Flutter Mane + Urshifu'])
    expect(filterEnemyLeadStats(leads, '', 2).map(s => s.lead)).toEqual(['Incineroar + Rillaboom'])
  })
})
